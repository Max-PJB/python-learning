...
class Solution(object):
    def removeStones(self, stones):
        ...
        :rtype: int
        """
        # 这个题目之前看过，其实就是求不同的块，用总的石头减去块，就是了
        n = len(stones)
        if n < 2:
            return 0
        region = -n
        while stones:
            ...
            while stack:
                ...
                i = 0
                while i < n:
                    if stones[i][0] == node[0] or stones[i][1] == node[1]:
                        stack.append(stones.pop(i))
                        n -= 1
                    else:
                        i += 1
        return -region
    ...
    def removeStones2(self, stones):
        ...
        def dfs(id_stone):
            ...
            for i in range(n):
                if flag[i] == 0:
                    if stones[i][0] == stones[id_stone][0] or stones[i][1] == stones[id_stone][1]:
                        dfs(i)
